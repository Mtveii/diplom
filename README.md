...
Вход: через страницу `/login` (логин/email + пароль, `POST /api/Auth/login`). Токен хранится в `localStorage` с проверкой `exp` и сбрасывается при `401`. Выход через меню пользователя или `/profile`.

Переменные окружения Frontend: см. `Frontend/.env.example`. Никаких `VITE_AUTH_EMAIL/_PASSWORD` в прод-бандле — это была уязвимость (креды в JS).
